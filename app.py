...
@app.route('/')
def list_sims():
    #sim_ids = os.listdir(app.config['UPLOAD_FOLDER'])
...
#@app.route('/')
#def index():
#    sims = os.listdir(app.config['UPLOAD_FOLDER'])
#    return render_template('index.html', sims=sims)
...
def add_sim():
    ...
    if file and allowed_file(file.filename):
        ...
        sim_path = os.path.join(app.config['UPLOAD_FOLDER'], sim_id)
...
@app.route('/delete_sim/<sim_id>', methods=['POST','GET'])
...
@app.route('/update_sim/<sim_id>', methods=['GET', 'POST'])
...
#@app.route('/sims/<sim_id>')
#def view_sim(sim_id):
#    return send_from_directory(os.path.join(app.config['UPLOAD_FOLDER'], sim_id), 'index.html')
@app.route('/get_sim/<sim_id>')
def get_sim(sim_id):
    with open('sim.json', 'r') as f:
        sims = json.load(f)
...
@app.route('/sims/<sim_id>/styles.css')
def sim_css(sim_id):
    return send_from_directory(os.path.join(app.config['UPLOAD_FOLDER'], sim_id), 'styles.css', mimetype='text/css')

@app.route('/sims/<sim_id>/script.js')
def sim_js(sim_id):
    return send_from_directory(os.path.join(app.config['UPLOAD_FOLDER'], sim_id), 'script.js', mimetype='application/javascript')
